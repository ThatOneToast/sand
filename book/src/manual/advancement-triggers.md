...
`AdvancementTrigger::RecipeCrafted { recipe_id, ingredients }`; vanilla
requires the concrete recipe ID and does not expose an unfiltered “any crafted
item” trigger.
...
**Verified against real Minecraft 1.21.4 and 26.2 servers:** the exact JSON
shape above was generated by this crate, loaded, and `reload`-tested via
`sand-vanilla-audit` (`audit_placed_block_filtered`,
`audit_item_used_on_block_filtered`). A connected protocol client additionally
verifies 1.21.4 positive and negative matches, final-item-in-stack behavior,
and a second firing after reward-function revoke/reset. See
`docs/vanilla-reload-validation.md`. The 26.2 evidence remains parse/reload
evidence only because the semantic client does not support that protocol.
...
distinguishes four levels of evidence per trigger:

- `golden_json_tested` — a fixed-input/fixed-output serialization test exists.
- `schema_golden_tested_profiles` — profile-specific rendering branches have
  fixed expected JSON.
...
`vanilla_load_tested_profiles: &["1.21.4", "26.2"]` and
`semantic_runtime_tested_profiles: &["1.21.4"]`. Do not read `Typed` API
status, a golden test, or reload success as semantic proof for another trigger
or profile.
