...
| `ItemCraftEvent` | `sand_core::event::vanilla::AnyItemCrafted` | Legacy compatibility marker | Removed `minecraft:crafted_item` | Unsupported on verified current profiles | Unavailable | — | Current `minecraft:recipe_crafted` requires a concrete `recipe_id`, so Sand cannot truthfully implement an unfiltered “any craft” wrapper. Use `AdvancementTrigger::RecipeCrafted { recipe_id, ingredients }`. |
...
| `BucketEmptyEvent` | `sand_core::events::BucketEmptyEvent` | Legacy compatibility marker | Unregistered `minecraft:emptied_bucket` | Unsupported on verified profiles | Unavailable | — | Target-aware export rejects this stale trigger. No exact current replacement has been verified. |
...
| `ItemPickedUpEvent` | `sand_core::events::ItemPickedUpEvent` | React when the player picks up a thrown item | `minecraft:thrown_item_picked_up_by_player` | Advancement (AfterFire) | High | Negligible | Only fired for items that were previously thrown; the separate `ThrownItemPickedUpByEntity` variant covers non-player collectors. |
...
| Event type | Preferred import | User intent | Vanilla mechanism | Dispatch | Reliability | Cost | Caveats |
|---|---|---|---|---|---|---|---|
| `BlockPlaceEvent` | `sand_core::event::vanilla::AnyBlockPlaced` | React when the player places a block | `minecraft:placed_block` | Advancement (AfterFire) | High | Negligible | No block filter on the built-in type. For a filtered custom `SandEvent` built directly on `AdvancementTrigger::placed_block(...)`/`ItemUsedOnBlock`, filtering renders per-profile through `AdvancementSchemaFamily` — see [Advancement Triggers](../manual/advancement-triggers.md#version-aware-rendering-placed_block-and-item_used_on_block). Vanilla-load-verified on 1.21.4 and 26.2; positive/negative, final-stack, and revoke/re-fire semantics verified by a protocol client on 1.21.4. |
