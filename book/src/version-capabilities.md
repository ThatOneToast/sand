...
| `systems-player-data` | manual `PlayerDataSchema`/`PlayerSchema` builder helpers; implies lifecycle |
...
`systems-player-data` currently exposes the `PlayerDataSchema` builder
(`PlayerSchema` remains as an alias). It groups score/flag/timer/cooldown
fields and storage descriptors, but users still call `define_all()` from load
and `init_player(selector)` from join or first-join handlers. Automatic
lifecycle/export wiring is future #47/#68 work.
