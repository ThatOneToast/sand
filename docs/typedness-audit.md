...
| `AdvancementTrigger` (all variants) | **Mostly typed** | Typed predicates plus validated associated constructors for resource-bearing variants; legacy string-field variants remain compatibility/raw paths. `PlacedBlock`/`ItemUsedOnBlock` render through `render_for(caps)`, a fallible profile-aware boundary that maps the target profile to an explicit `AdvancementSchemaFamily` (`LocationConditionItemComponents` vs `Legacy`) in one place and selects the vanilla-correct `location_check`/`match_tool` condition schema per family instead of one unconditional `Serialize` shape (#231, #232, #233). Item-predicate conversion is tagged with an `AdvancementItemConsumer` (`PlacedBlockTool`/`ItemUsedOnBlockTool`) for diagnostics; requesting an item filter on `Legacy` fails loudly rather than emitting an unrecognized schema. `sand_components::advancement::trigger_coverage::TriggerCoverage` now tracks `vanilla_load_tested_profiles`/`semantic_runtime_tested_profiles` per trigger separately from `golden_json_tested` — `placed_block`/`item_used_on_block` have real 26.2 vanilla load/reload evidence but no automated semantic (fires-only-for-matches) evidence yet; see `docs/vanilla-reload-validation.md` |
