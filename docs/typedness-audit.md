...
| `AdvancementTrigger` (all variants) | **Mostly typed** | Target-aware export routes every typed trigger through the resolved `VersionCaps` and an explicit `AdvancementSchemaFamily`. Consumer-aware item, location, entity, and damage rendering handles the verified 1.21.4/26.2 schema differences; unsupported conversions fail rather than dropping a filter. Six stale source-compatibility trigger IDs are rejected, while `Custom`/raw predicates remain verbatim and user-owned. `TriggerCoverage` separately records typed API, generic/schema golden output, real-server load, and client-driven semantic evidence; only `placed_block` and `item_used_on_block` currently have the last tier, on 1.21.4. See `ai/known-limitations.md` (LIM-API-010, LIM-VAL-003) and `docs/vanilla-reload-validation.md`. |
