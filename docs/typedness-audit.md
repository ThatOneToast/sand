...
| `villager_trade`/`trade_set` registries and the Villager Trade tag pools | `RawOnly` (`RawComponent`/raw JSON only) | `villager_trade::{VillagerTrade, TradeSet, VillagerTradePoolPatch}`, typed `TradeItem`, reused `ItemStack`/`EnchantmentSelection`, typed `VillagerProfession`/`VillagerLevel`/`WanderingTraderPool`/`VillagerTradePool` known-pool targets, `TagId<VillagerTradeId>`, and the new `DatapackComponent::nested_components` compound-export hook (`TradeSet::entry`/`VillagerTradePoolPatch::append` hoist inline trades into deterministic generated `villager_trade` resources) | narrow first pass covering the common/stable trade shape (costs, result, `max_uses`, `reputation_discount`, `merchant_xp`, double-price enchantment selection); `given_item_modifiers` and `merchant_predicate` remain raw JSON escape hatches pending #185/#204; #296 |
...
| recipe result identity for component-bearing custom items | `RecipeResult { id: String, count: u32 }` only — no way to carry a `CustomItem`'s data components into a recipe result | `RecipeResult::custom_item`/`from_custom_item` (+ `TryFrom<CustomItem>`/`TryFrom<&CustomItem>`, `CustomItem::recipe_result`) build a `result.components` object by reusing `CustomItem::stack_components`'s typed component serialization; gated on `ComponentFeature::ItemComponents` so unsupported targets fail clearly instead of stripping components; component-free `RecipeResult::item`/`raw`/`new` output is byte-identical to before | completed for results; component-aware recipe *ingredients* are intentionally unsupported — `Ingredient::custom_item` always returns a capability error because no targeted Minecraft version matches recipe ingredients by data component (silently degrading to base-item matching was identified as the primary risk to avoid); #226 |
| jukebox_song / instrument description | `description` field typed as bare `serde_json::Value` behind an *unnamed* setter (no typed text path, no export-time validation) | pending: typed `TextComponent`-backed `description()` plus an explicitly named `raw_description()` escape hatch, mirroring `TrimMaterial`/`AdvancementDisplay` | found overstated as `FullyImplemented` during #193's audit; downgraded to `PartiallyImplemented`; #321 |
| trim_material override_armor_assets | only the legacy pre-1.21.4 `override_armor_materials` shape is modeled | pending: typed `override_armor_assets` builder matching the current vanilla field | found during #193's audit; `minecraft:trim_material` remains `PartiallyImplemented`; #322 |
| configured-carver reference and payload | raw `serde_json::Value` carvers field on `Biome`, no builder for `worldgen/configured_carver/*.json` (`Missing` in `registry_coverage.rs`) | `ConfiguredCarverId`, `ConfiguredCarver` typed constructors for `cave`/`nether_cave` sharing `CaveCarverConfig` (reusing `HeightProvider`/`VerticalAnchor`, plus a new `CarverFloatRange` uniform-float provider), `Biome::carver_step` (typed, step-grouped via `CarvingStep`), explicit `ConfiguredCarver::raw`/`Biome::raw_carvers` escape hatches | completed typed reference path + small common builder slice; other vanilla carver shapes remain raw pending broader worldgen parity; #191 |
| `worldgen/biome` ambient sound, temperature modifier, and per-step feature references | raw `String` ambient sound, raw `String` temperature modifier (validated but untyped), raw `Value` features list-of-lists | `SoundEventId`-typed `BiomeEffects::ambient_sound`, `TemperatureModifier` enum (`None`/`Frozen`) for `Biome::temperature_modifier`, and `Biome::feature(GenerationStep, ConfiguredFeatureId)` building the per-generation-step list-of-lists, with `BiomeEffects::raw_ambient_sound`/`Biome::raw_temperature_modifier`/`Biome::raw_features` escape hatches | completed this slice of #182 plus `Biome::carvers` typing (#191); `spawners`/`spawn_costs` typing and the rest of #182 (dimension generators, noise settings, placement modifiers) remain open |
