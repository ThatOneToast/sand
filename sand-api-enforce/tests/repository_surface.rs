...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=26 pending_items=4978 enforced_items=6543 pending_scope_ceiling=26 pending_item_ceiling=4978"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    for (version, total, commands, registries) in expected {
        ...
        assert_eq!(
            ...
            total - 421 - enforced_registries - commands
        );
