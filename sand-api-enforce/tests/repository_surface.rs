...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    assert_eq!(lines[3], "total=11091");

    let kinds = prefixed_counts(&lines, "kind ");
    assert_eq!(kinds.values().sum::<usize>(), 11_091);
    assert_eq!(kinds["field"], 707);
    assert_eq!(kinds["variant"], 5_752);
    ...
    assert_eq!(origins.values().sum::<usize>(), 11_091);
    assert_eq!(origins["source"], 4_723);
    ...
    let scoped_items = scope_lines
        ...
        .sum::<usize>();
    assert_eq!(scoped_items, 11_091);
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=0 pending_items=0 enforced_items=11091 pending_scope_ceiling=0 pending_item_ceiling=0"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    let expected = [
        ("placeholder-codegen", 4_991, 0, 0, 0),
        ("1.21.4", 10_181, 902, 4_288, 0),
        ("26.2", 11_091, 1_233, 4_867, 0),
    ];
