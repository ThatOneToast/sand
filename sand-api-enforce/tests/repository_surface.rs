...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    assert_eq!(origins.values().sum::<usize>(), 10_869);
    assert_eq!(origins["source"], 4_495);
    ...
    let scope_lines = lines
        ...
        .collect::<Vec<_>>();
    assert_eq!(scope_lines.len(), 27);
    let scoped_items = scope_lines
        .iter()
        .map(|line| numeric_field(line, "items="))
        .sum::<usize>();
    assert_eq!(scoped_items, 10_869);
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=0 pending_items=0 enforced_items=10869 pending_scope_ceiling=0 pending_item_ceiling=0"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    let expected = [
        ("placeholder-codegen", 4_769, 0, 0, 0),
        ("26.2", 10_869, 1_233, 4_867, 0),
    ];
