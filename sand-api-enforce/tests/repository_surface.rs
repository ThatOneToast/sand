...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    assert_eq!(lines[3], "total=11063");

    let kinds = prefixed_counts(&lines, "kind ");
    assert_eq!(kinds.values().sum::<usize>(), 11_063);
    assert_eq!(kinds["field"], 771);
    ...
    assert_eq!(origins.values().sum::<usize>(), 11_063);
    assert_eq!(origins["source"], 4_685);
    ...
    let scoped_items = scope_lines
        ...
        .sum::<usize>();
    assert_eq!(scoped_items, 11_063);
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=10 pending_items=3975 enforced_items=7088 pending_scope_ceiling=10 pending_item_ceiling=3975"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    let expected = [
        ("placeholder-codegen", 4_941, 0, 0, 3_975),
        ("1.21.4", 10_153, 924, 4_288, 3_975),
        ("26.2", 11_063, 1_255, 4_867, 3_975),
    ];
