...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    assert_eq!(lines[3], "total=11736");

    let kinds = prefixed_counts(&lines, "kind ");
    assert_eq!(kinds.values().sum::<usize>(), 11_736);
    assert_eq!(kinds["field"], 1_047);
    assert_eq!(kinds["variant"], 5_896);
    assert_eq!(kinds["attribute_macro"], 8);
    assert_eq!(kinds["derive_macro"], 3);

    let origins = prefixed_counts(&lines, "origin ");
    assert_eq!(origins.values().sum::<usize>(), 11_736);
    assert_eq!(origins["source"], 5_338);
    ...
    let scoped_items = scope_lines
        ...
        .sum::<usize>();
    assert_eq!(scoped_items, 11_736);
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=38 pending_items=11613 enforced_items=123 pending_scope_ceiling=38 pending_item_ceiling=11613"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    let expected = [
        ("placeholder-codegen", 5_614, 0, 0),
        ("1.21.4", 10_826, 924, 4_288),
        ("26.2", 11_736, 1_255, 4_867),
    ];
    for (version, total, commands, registries) in expected {
        ...
        assert_eq!(profile.pending_item_ceiling, total - 123);
