...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=27 pending_items=5003 enforced_items=6518 pending_scope_ceiling=27 pending_item_ceiling=5003"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    for (version, total, commands, registries) in expected {
        ...
        assert_eq!(
            ...
            total - 396 - enforced_registries - commands
        );
