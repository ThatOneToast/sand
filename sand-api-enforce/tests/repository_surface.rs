...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    let scoped_items = scope_lines
        ...
        .sum::<usize>();
    assert_eq!(scoped_items, 11_150);
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=0 pending_items=0 enforced_items=11150 pending_scope_ceiling=0 pending_item_ceiling=0"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    let expected = [
        ("placeholder-codegen", 5_050, 0, 0, 0),
        ("1.21.4", 10_240, 902, 4_288, 0),
        ("26.2", 11_150, 1_233, 4_867, 0),
    ];
