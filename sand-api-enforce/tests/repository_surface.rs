...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    assert_eq!(lines[3], "total=10976");

    let kinds = prefixed_counts(&lines, "kind ");
    assert_eq!(kinds.values().sum::<usize>(), 10_976);
    assert_eq!(kinds["field"], 701);
    assert_eq!(kinds["variant"], 5_749);
    assert_eq!(kinds["enum"], 150);
    assert_eq!(kinds["trait"], 35);
    assert_eq!(kinds["trait_method"], 51);
    assert_eq!(kinds["attribute_macro"], 8);
    assert_eq!(kinds["derive_macro"], 3);

    let origins = prefixed_counts(&lines, "origin ");
    assert_eq!(origins.values().sum::<usize>(), 10_976);
    assert_eq!(origins["source"], 4_608);
    ...
    let scoped_items = scope_lines
        ...
        .sum::<usize>();
    assert_eq!(scoped_items, 10_976);
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=0 pending_items=0 enforced_items=10976 pending_scope_ceiling=0 pending_item_ceiling=0"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    let expected = [
        ("placeholder-codegen", 4_876, 0, 0, 0),
        ("1.21.4", 10_066, 902, 4_288, 0),
        ("26.2", 10_976, 1_233, 4_867, 0),
    ];
