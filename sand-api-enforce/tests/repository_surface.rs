...
fn checked_repository_surface_baseline_is_complete_and_partitioned() {
    ...
    assert_eq!(lines[3], "total=11178");

    let kinds = prefixed_counts(&lines, "kind ");
    assert_eq!(kinds.values().sum::<usize>(), 11_178);
    ...
    assert_eq!(origins.values().sum::<usize>(), 11_178);
    assert_eq!(origins["source"], 4_810);
    ...
    let scoped_items = scope_lines
        ...
        .sum::<usize>();
    assert_eq!(scoped_items, 11_178);
    assert_eq!(
        lines.last().copied(),
        Some(
            "totals pending_scopes=0 pending_items=0 enforced_items=11178 pending_scope_ceiling=0 pending_item_ceiling=0"
        )
    );
}
...
fn checked_repository_profiles_bind_exact_versioned_baselines() {
    ...
    let expected = [
        ("placeholder-codegen", 5_078, 0, 0, 0),
        ("1.21.4", 10_268, 902, 4_288, 0),
        ("26.2", 11_178, 1_233, 4_867, 0),
    ];
