...
impl ResolvedServerProperties {
    fn resolve(offline_flag: bool, server_config: Option<&ServerConfigFile>) -> Self {
        let online_mode = if offline_flag {
            ...
            server_config.is_none_or(|s| s.online_mode)
        };
        Self {
            online_mode,
            view_distance: server_config.map_or(10, |s| s.view_distance),
            simulation_distance: server_config.map_or(10, |s| s.simulation_distance),
            difficulty: server_config.map_or("normal", |s| match s.difficulty.as_str() {
                "peaceful" => "peaceful",
                "easy" => "easy",
                "hard" => "hard",
                _ => "normal",
...
pub fn run(args: RunArgs) -> Result<()> {
    ...
    if server_config
        .as_ref()
        .is_some_and(|s| s.world_reset_policy_always_reset)
    {
...
mod tests {
    ...
    fn server_config(
        ...
        ServerConfigFile {
            ...
            online_mode,
            world_reset_policy_always_reset,
        }
    }
