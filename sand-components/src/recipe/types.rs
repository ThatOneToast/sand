...
impl Ingredient {
    ...
    /// targets (`26.x+`). There is no component predicate in the recipe
    ...
    #[sand_macros::api(
        ...
        context = "Attempt to build a recipe ingredient that matches a specific [`CustomItem`] exactly, including its data components (e.g. its [`custom_data`](CustomItem::custom_data) identity marker). Minecraft's vanilla crafting recipe `Ingredient` schema matches only by item ID or item tag throughout Sand's supported 26.x+ targets. There is no component predicate in the recipe ingredient schema; component predicates exist only in predicate and advancement JSON, which the crafting grid does not consult. Silently degrading to an item-ID-only ingredient would lose the custom item's identity, so this returns a validation error. If matching by base item type alone is acceptable, use [`Ingredient::item`].",
        minecraft = "Minecraft's vanilla crafting recipe `Ingredient` schema matches only by item ID or item tag throughout Sand's supported 26.x+ targets; it does not accept component predicates.",
        use_when = ["Defining a typed advancement, recipe, loot table, worldgen resource, item property, or related datapack component"],
        avoid_when = ["Injecting unchecked JSON when the typed schema can represent the resource"],
