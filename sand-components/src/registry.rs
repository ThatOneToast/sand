...
registry_id! {
    @contract(
        ...
        avoid_when = ["Building the dialog payload itself", "Targeting an unverified future Minecraft schema"],
        example_namespace = "demo",
...
registry_id! {
    ...
    /// `minecraft:cold` or `mymod:arcane_chicken`).
    ChickenVariantId
}

registry_id! {
    /// Typed identifier for the `cow_variant` registry (e.g.
    /// `minecraft:warm` or `mymod:arcane_cow`).
    CowVariantId
}

registry_id! {
    /// Typed identifier for the `pig_variant` registry (e.g.
    /// `minecraft:cold` or `mymod:arcane_pig`).
    PigVariantId
}
...
registry_id! {
    ...
    /// Part of Sand's Minecraft 26+ registry baseline.
    VillagerTradeId
}
