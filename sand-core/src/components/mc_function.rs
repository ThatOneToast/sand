...
impl DatapackComponent for McFunction {
    ...
    fn to_json(&self) -> Value {
        Value::Array(self.commands.iter().map(|c| Value::String(c.clone())).collect())
    }
    ...
    fn component_dir(&self) -> &'static str { "function" }
    fn file_extension(&self) -> &'static str { "mcfunction" }
}
