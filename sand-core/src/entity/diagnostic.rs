...
pub enum EntityDiagnostic {
    /// An archetype behavior references State that is not in its composition.
    #[error(
        "[SAND-ENTITY-COMPONENT-MISSING] archetype `{archetype}` {property} references `{component}::{field}`, but component `{component}` is not part of this archetype"
    )]
    MissingArchetypeComponent {
        /// Archetype resource identifier.
        archetype: String,
        /// Property or derivation that references the field.
        property: String,
        /// Missing State component identity.
        component: String,
...
impl EntityDiagnostic {
    ...
    pub const fn code(&self) -> &'static str {
        match self {
            Self::MissingArchetypeComponent { .. } => "SAND-ENTITY-COMPONENT-MISSING",
