...
impl HealthBinding {
    ...
    #[sand_macros::api(
        ...
        minecraft = "ObserveNative reads native Health into the score without subscribing to score dirtiness. ApplyState subscribes to dirty score changes and writes them to native Health. Bidirectional does both, snapshots the state-write cause before shared dirtiness is consumed, and gives an explicit pending State write precedence over native observation in the same pass. observe_native_every adds bounded periodic observation for native changes that Sand did not initiate.",
