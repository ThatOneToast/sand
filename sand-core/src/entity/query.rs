...
/// declarations. The public-facing system-query proof below additionally
/// requires the declaration to have a queryable (non-global) scope.
#[doc(hidden)]
pub trait GeneratedSystemQueryParameter: 'static {}

/// Proof that a type is both generated by Sand and queryable by a system.
#[doc(hidden)]
#[diagnostic::on_unimplemented(
    message = "[SAND-SYSTEM-PARAM] `{Self}` is not a scoped State, StateBundle, or StateQuery",
    label = "use a concrete query type generated by Sand"
)]
pub trait SystemQueryParameter: StateQuerySpec + 'static {}

impl<Q> SystemQueryParameter for Q where Q: StateQuerySpec + GeneratedSystemQueryParameter {}
...
#[diagnostic::on_unimplemented(
    message = "[SAND-SYSTEM-PARAM] `{Self}` is not a scoped State, StateBundle, or StateQuery",
    label = "#[system] query parameters must implement StateQueryOperations"
)]
#[sand_macros::api(
...
impl<Q: SystemQueryParameter> StateQueryOperations for Q {}

/// Assert that a declared `#[system]` query parameter is one of Sand's
/// generated concrete query types, even when the authored body does not use it.
#[doc(hidden)]
pub fn assert_system_query_parameter<Q: SystemQueryParameter>() {}
