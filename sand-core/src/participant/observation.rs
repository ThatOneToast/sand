...
impl ObservationSchema {
    ...
    #[sand_macros::api(
        ...
        use_when = ["Declaring or reading a typed participant whose lifecycle is guaranteed by the event plan"],
        avoid_when = ["Assuming an entity or item remains live beyond its declared invocation, event-cycle, or bounded correlation lifetime"],
        params(storage = "`storage` supplies the storage value used to create the storage schema used to correlate participant observations for an event.", event_label = "`event_label` supplies the event label value used to create the storage schema used to correlate participant observations for an event."),
        returns = "A newly constructed `ObservationSchema` configured to create the storage schema used to correlate participant observations for an event.",
