...
impl Interactable {
    ...
    #[sand_macros::api(
        ...
        returns = "A newly constructed `Interactable` configured to create a new `Interactable` builder. `advancement_location` is the resource location registered via `#[datapack_component]` for the resulting advancement.",
    ...
    #[sand_macros::api(
        ...
        use_when = ["Opting into the documented higher-level gameplay behavior instead of assembling its commands manually"],
        avoid_when = ["Using the API outside its documented system scope or feature configuration"],
        params(size = "`size` supplies the size value used to set the hitbox dimensions (default: 1×1 blocks)."),
    ...
    #[sand_macros::api(
        ...
        avoid_when = ["Using the API outside its documented system scope or feature configuration"],
        params(tag = "`tag` supplies the tag value used to filter by a tag that must be present on the interaction entity."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Opting into the documented higher-level gameplay behavior instead of assembling its commands manually"],
        avoid_when = ["Using the API outside its documented system scope or feature configuration"],
        params(pos = "`pos` supplies the pos value used to emit the documented `summon minecraft:interaction <pos> {width:W,height:H,response:1b[,Tags:[...]]}` form."),
