//! An advancement-backed graph parent that declares setup objectives is
//! rejected (#240 Phase 6): the synchronous bridge dispatches the dependent
//! directly from the parent's own reward entry function and never runs the
//! parent's own `SandEvent::setup()` lifecycle, so a non-empty setup would
//! be silently dropped rather than executed. Isolated in its own test
//! binary since `inventory` registrations are process-global.
...
impl SandEvent for AdvancementParent {
    ...
    fn setup() -> EventSetup {
        EventSetup {
            objectives: vec!["scoreboard objectives add p6_setup_obj dummy".into()],
            pre_observation: vec![],
...
#[test]
fn advancement_parent_with_setup_objectives_is_rejected() {
    let error = sand_core::try_export_components_json("advsetupobjrejected").expect_err(
    ...
    assert!(
        !message.to_lowercase().contains("unsupported event"),
        "error must not be a generic message: {message}"
    );
}
