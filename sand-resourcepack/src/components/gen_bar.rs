...
#[sand_macros::api(
    ...
    path = "sand::resourcepack::GenHudBar",
    module = "sand::resourcepack",
    summary = "A bitmap-font progress bar whose sprite strip is generated at build time from color parameters rather than copied from a user-supplied PNG.",
    context = "A bitmap-font progress bar whose sprite strip is generated at build time from color parameters rather than copied from a user-supplied PNG. Constructed via the `hud_bar!` macro with a `create!(...)` expression in the `texture:` field: The macro expands to a [`GenHudBar`] instead of a [`HudBar`] when it detects the `create!(...)` call. [`HudBar`]: sand::resourcepack::HudBar",
    ...
    example = "use sand::resourcepack::GenHudBar;",
    availability = ["Cargo feature: resourcepack"],
    fields(ascent = "Vertical offset from the baseline to the top of the glyph.", empty = "Packed `0xRRGGBBAA` color for the empty / background portion.", fill = "Packed `0xRRGGBBAA` color for the filled portion of each frame.", font = "Name of the font file (without extension).", frame_width = "Pixel width of each individual frame. A value of `0` means \"use `height`\" (produces square frames).", height = "Rendered glyph height in pixels. Also used as `frame_width` when `frame_width` is `0`.", name = "Unique identifier used in diagnostics and codepoint derivation.", steps = "Number of frames in the generated sprite strip.", texture_dest = "Destination sub-path inside `assets/<namespace>/textures/` (without extension), e.g. `\"font/health_bar\"`.", unicode_start = "Override the first unicode codepoint for the bar frames."),
)]
