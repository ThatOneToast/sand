...
impl BarHandle {
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(frame = "Minecraft JSON text component string for `frame`.", namespace = "`namespace` sets the namespace for minecraft JSON text component string for `frame`."),
    ...
    #[sand_macros::api(
        ...
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(target = "Returns a single `title <target> actionbar …` command that shows frame `frame` of this bar to `target`.", frame = "Returns a single `title <target> actionbar …` command that shows frame `frame` of this bar to `target`.", namespace = "`namespace` is used to return a single `title <target> actionbar …` command that shows frame `frame` of this bar to `target`."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(target = "`target` provides the entity, block, or command target used to use like [`show`], but shifts the bar by `x_offset` font pixels from screen center (positive = right, negative = left).", frame = "`frame` sets the frame for like [`show`], but shifts the bar by `x_offset` font pixels from screen center (positive = right, negative = left).", namespace = "`namespace` sets the namespace for like [`show`], but shifts the bar by `x_offset` font pixels from screen center (positive = right, negative = left).", x_offset = "Like [`show`], but shifts the bar by `x_offset` font pixels from screen center (positive = right, negative = left)."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(target = "`target` provides the entity, block, or command target used to use like [`show_at`], but takes a virtual canvas position instead of a raw font-pixel offset.", frame = "`frame` sets the frame for like [`show_at`], but takes a virtual canvas position instead of a raw font-pixel offset.", namespace = "`namespace` sets the namespace for like [`show_at`], but takes a virtual canvas position instead of a raw font-pixel offset.", canvas_x = "`canvas_x` sets the canvas x for like [`show_at`], but takes a virtual canvas position instead of a raw font-pixel offset.", canvas_width = "`canvas_width` sets the canvas width for like [`show_at`], but takes a virtual canvas position instead of a raw font-pixel offset."),
    ...
    #[sand_macros::api(
        ...
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(holder = "`holder` is used to return one `execute if score … run title …` command per frame.", objective = "`objective` is used to return one `execute if score … run title …` command per frame.", namespace = "`namespace` is used to return one `execute if score … run title …` command per frame."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(holder = "`holder` sets the holder for like [`display_commands`], but shifts each frame by `x_offset` pixels.", objective = "`objective` sets the objective for like [`display_commands`], but shifts each frame by `x_offset` pixels.", namespace = "`namespace` sets the namespace for like [`display_commands`], but shifts each frame by `x_offset` pixels.", x_offset = "Like [`display_commands`], but shifts each frame by `x_offset` pixels."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(holder = "`holder` sets the holder for like [`display_commands`], but positions each frame using a virtual canvas coordinate. See [`show_at_canvas`] for how canvas coordinates work.", objective = "`objective` sets the objective for like [`display_commands`], but positions each frame using a virtual canvas coordinate. See [`show_at_canvas`] for how canvas coordinates work.", namespace = "`namespace` sets the namespace for like [`display_commands`], but positions each frame using a virtual canvas coordinate. See [`show_at_canvas`] for how canvas coordinates work.", canvas_x = "`canvas_x` sets the canvas x for like [`display_commands`], but positions each frame using a virtual canvas coordinate. See [`show_at_canvas`] for how canvas coordinates work.", canvas_width = "`canvas_width` sets the canvas width for like [`display_commands`], but positions each frame using a virtual canvas coordinate. See [`show_at_canvas`] for how canvas coordinates work."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(executor = "`executor` is typically `\"@a\"`. Minecraft runs once per matched player, so each player sees the frame matching their own score.", objective = "`objective` is used to return one `execute as … if score @s … run title @s actionbar …` command per frame, for use in tick functions.", namespace = "`namespace` is used to return one `execute as … if score @s … run title @s actionbar …` command per frame, for use in tick functions."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(executor = "`executor` sets the executor for like [`broadcast_commands`], but shifts each frame by `x_offset` font pixels.", objective = "`objective` sets the objective for like [`broadcast_commands`], but shifts each frame by `x_offset` font pixels.", namespace = "`namespace` sets the namespace for like [`broadcast_commands`], but shifts each frame by `x_offset` font pixels.", x_offset = "Like [`broadcast_commands`], but shifts each frame by `x_offset` font pixels."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(executor = "`executor` sets the executor for like [`broadcast_commands`], but positions using a virtual canvas coordinate.", objective = "`objective` sets the objective for like [`broadcast_commands`], but positions using a virtual canvas coordinate.", namespace = "`namespace` sets the namespace for like [`broadcast_commands`], but positions using a virtual canvas coordinate.", canvas_x = "`canvas_x` sets the canvas x for like [`broadcast_commands`], but positions using a virtual canvas coordinate.", canvas_width = "`canvas_width` sets the canvas width for like [`broadcast_commands`], but positions using a virtual canvas coordinate."),
...
impl ElementHandle {
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(namespace = "`namespace` sets the namespace for minecraft JSON text component string for this element."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(target = "`target` provides the entity, block, or command target used to return a single `title <target> actionbar …` command.", namespace = "`namespace` is used to return a single `title <target> actionbar …` command."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(target = "`target` provides the entity, block, or command target used to use like [`show`], but shifts the element by `x_offset` font pixels from center.", namespace = "`namespace` sets the namespace for like [`show`], but shifts the element by `x_offset` font pixels from center.", x_offset = "Like [`show`], but shifts the element by `x_offset` font pixels from center."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(target = "`target` provides the entity, block, or command target used to use like [`show`], but positions the element using a virtual canvas coordinate.", namespace = "`namespace` sets the namespace for like [`show`], but positions the element using a virtual canvas coordinate.", canvas_x = "`canvas_x` sets the canvas x for like [`show`], but positions the element using a virtual canvas coordinate.", canvas_width = "`canvas_width` sets the canvas width for like [`show`], but positions the element using a virtual canvas coordinate."),
