...
impl HudLayout {
    ...
    #[sand_macros::api(
        ...
        returns = "A `HudLayout` representing a new layout for `namespace` with a virtual canvas of `canvas_width` units wide.",
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(stat = "`stat` provides the stat added when building a [`BarStat`]-tracked bar to the layout. Equivalent to `.bar(stat.handle, &stat.frame_obj, canvas_x)` — this is a convenience shortcut so you don't have to spell out the objective name yourself when using the `BarStat` API.", canvas_x = "`canvas_x` provides the canvas x added when building a [`BarStat`]-tracked bar to the layout. Equivalent to `.bar(stat.handle, &stat.frame_obj, canvas_x)` — this is a convenience shortcut so you don't have to spell out the objective name yourself when using the `BarStat` API."),
    ...
    #[sand_macros::api(
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(executor = "`executor` selects the players that receive the storage-based HUD broadcast: O(N₁ + N₂ + … + 1) commands instead of the Cartesian-product O(N₁ × N₂ × …) produced by [`broadcast`]."),
