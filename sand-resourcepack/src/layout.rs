...
#[sand_macros::api(
    ...
    path = "sand::resourcepack::HudLayout",
    module = "sand::resourcepack",
    summary = "Composes multiple HUD bars and elements into a single actionbar command set.",
    context = "Composes multiple HUD bars and elements into a single actionbar command set. See the [module-level documentation](sand::resourcepack) for a full walkthrough. | Method | When to use | |---|---| | [`broadcast`] | `#[datapack_component(Tick)]` — iterates over all players | | [`show_elements`] | Show only static elements (no bar state needed) | [`broadcast`]: HudLayout::broadcast [`show_elements`]: HudLayout::show_elements",
    minecraft = "| Method | When to use | |---|---| | [`broadcast`] | `#[datapack_component(Tick)]` — iterates over all players | | [`show_elements`] | Show only static elements (no bar state needed) |",
    ...
    example = "use sand::resourcepack::HudLayout;",
...
impl HudLayout {
    ...
    #[sand_macros::api(
        registry = sand_api_contract,
        path = "sand::resourcepack::HudLayout::new",
        module = "sand::resourcepack",
        kind = "method",
        summary = "Create a new layout for `namespace` with a virtual canvas of `canvas_width` units wide.",
        context = "Create a new layout for `namespace` with a virtual canvas of `canvas_width` units wide. `canvas_width / 2` maps to the horizontal screen center. For example, with `canvas_width = 1000`, `canvas_x = 0` is the far left, `500` is the center, and `1000` is the far right. Any value works; `1000` is a convenient round number. To make \"1 unit = 1 visible pixel\" at a specific resolution/scale combination: | Resolution | GUI scale | `canvas_width` | |---|---|---| | 1920 × 1080 | 2 | 960 | | 1920 × 1080 | 3 | 640 | | 2560 × 1440 | 2 | 1280 |",
        ...
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(namespace = "Create a new layout for `namespace` with a virtual canvas of `canvas_width` units wide.", canvas_width = "Create a new layout for `namespace` with a virtual canvas of `canvas_width` units wide."),
        returns = "A newly constructed `HudLayout` configured to create a new layout for `namespace` with a virtual canvas of `canvas_width` units wide.",
        example = "use sand::prelude::*;\n\nfn demonstrate(namespace: impl Into < String >, canvas_width: i32)  {\n    let hud_layout = sand::resourcepack::HudLayout::new(namespace, canvas_width);\n}",
        availability = ["Cargo feature: resourcepack"],
    )]
    ...
    #[sand_macros::api(
        registry = sand_api_contract,
        path = "sand::resourcepack::HudLayout::bar",
        module = "sand::resourcepack",
        kind = "method",
        summary = "Add a dynamic progress bar to the layout. - `handle` — the `BarHandle` constant generated by `hud_bar!` - `objective` — the scoreboard objective whose value selects the frame - `canvas_x` — horizontal position (0 = left edge, `canvas_width` = right edge).",
        context = "Add a dynamic progress bar to the layout. - `handle` — the `BarHandle` constant generated by `hud_bar!` - `objective` — the scoreboard objective whose value selects the frame - `canvas_x` — horizontal position (0 = left edge, `canvas_width` = right edge) The score at `objective` must be in `0..handle.steps` before the layout's commands run. Scale and clamp from your raw stat first.",
        minecraft = "- `handle` — the `BarHandle` constant generated by `hud_bar!` - `objective` — the scoreboard objective whose value selects the frame - `canvas_x` — horizontal position (0 = left edge, `canvas_width` = right edge)",
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(handle = "- `handle` — the `BarHandle` constant generated by `hud_bar!` - `objective` — the scoreboard objective whose value selects the frame - `canvas_x` — horizontal position (0 = left edge, `canvas_width` = right edge)", objective = "- `handle` — the `BarHandle` constant generated by `hud_bar!` - `objective` — the scoreboard objective whose value selects the frame - `canvas_x` — horizontal position (0 = left edge, `canvas_width` = right edge)", canvas_x = "- `handle` — the `BarHandle` constant generated by `hud_bar!` - `objective` — the scoreboard objective whose value selects the frame - `canvas_x` — horizontal position (0 = left edge, `canvas_width` = right edge)"),
        returns = "The `HudLayout` value with the documented change applied to add a dynamic progress bar to the layout. - `handle` — the `BarHandle` constant generated by `hud_bar!` - `objective` — the scoreboard objective whose value selects the frame - `canvas_x` — horizontal position (0 = left edge, `canvas_width` = right edge).",
        example = "use sand::prelude::*;\n\nfn demonstrate(hud_layout_value: sand::resourcepack::HudLayout, handle: sand::resourcepack::BarHandle, objective: impl Into < String >, canvas_x: i32)  {\n    let updated_hud_layout = hud_layout_value.bar(handle, objective, canvas_x);\n}",
        availability = ["Cargo feature: resourcepack"],
    )]
    ...
    #[sand_macros::api(
        registry = sand_api_contract,
        path = "sand::resourcepack::HudLayout::element",
        module = "sand::resourcepack",
        kind = "method",
        summary = "Add a static element to the layout. - `handle` — the `ElementHandle` constant generated by `hud_element!` - `canvas_x` — horizontal position.",
        context = "Add a static element to the layout. - `handle` — the `ElementHandle` constant generated by `hud_element!` - `canvas_x` — horizontal position",
        minecraft = "The resourcepack exporter writes version-appropriate assets, bitmap-font providers, and pack metadata for the selected Minecraft profile.",
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(handle = "- `handle` — the `ElementHandle` constant generated by `hud_element!` - `canvas_x` — horizontal position", canvas_x = "- `handle` — the `ElementHandle` constant generated by `hud_element!` - `canvas_x` — horizontal position"),
        returns = "The `HudLayout` value with the documented change applied to add a static element to the layout. - `handle` — the `ElementHandle` constant generated by `hud_element!` - `canvas_x` — horizontal position.",
        example = "use sand::prelude::*;\n\nfn demonstrate(hud_layout_value: sand::resourcepack::HudLayout, handle: sand::resourcepack::ElementHandle, canvas_x: i32)  {\n    let updated_hud_layout = hud_layout_value.element(handle, canvas_x);\n}",
        availability = ["Cargo feature: resourcepack"],
    )]
    ...
    #[sand_macros::api(
        registry = sand_api_contract,
        path = "sand::resourcepack::HudLayout::tracked_bar",
        module = "sand::resourcepack",
        kind = "method",
        summary = "Add a [`BarStat`]-tracked bar to the layout. Equivalent to `.bar(stat.handle, &stat.frame_obj, canvas_x)` — this is a convenience shortcut so you don't have to spell out the objective name yourself when using the `BarStat` API.",
        context = "Add a [`BarStat`]-tracked bar to the layout. Equivalent to `.bar(stat.handle, &stat.frame_obj, canvas_x)` — this is a convenience shortcut so you don't have to spell out the objective name yourself when using the `BarStat` API. This API defines client-side HUD, font, texture, or resource-pack output while keeping asset registration and exporter inventory wiring private.",
        minecraft = "The resourcepack exporter writes version-appropriate assets, bitmap-font providers, and pack metadata for the selected Minecraft profile.",
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(stat = "`stat` supplies the stat value used to add a [`BarStat`]-tracked bar to the layout. Equivalent to `.bar(stat.handle, &stat.frame_obj, canvas_x)` — this is a convenience shortcut so you don't have to spell out the objective name yourself when using the `BarStat` API.", canvas_x = "`canvas_x` supplies the canvas x value used to add a [`BarStat`]-tracked bar to the layout. Equivalent to `.bar(stat.handle, &stat.frame_obj, canvas_x)` — this is a convenience shortcut so you don't have to spell out the objective name yourself when using the `BarStat` API."),
        returns = "The `HudLayout` value with the documented change applied to add a [`BarStat`]-tracked bar to the layout. Equivalent to `.bar(stat.handle, &stat.frame_obj, canvas_x)` — this is a convenience shortcut so you don't have to spell out the objective name yourself when using the `BarStat` API.",
        example = "let health = BarStat::health(HEALTH);\nHudLayout::new(\"my_pack\", 1000)\n.tracked_bar(&health, 250)\n.broadcast(\"@a\");",
        availability = ["Cargo feature: resourcepack"],
    )]
    ...
    #[sand_macros::api(
        registry = sand_api_contract,
        path = "sand::resourcepack::HudLayout::broadcast",
        module = "sand::resourcepack",
        kind = "method",
        summary = "Generate all display commands for a tick function.",
        context = "Generate all display commands for a tick function. `executor` is the player selector, typically `\"@a\"`. For each player, Minecraft checks their scores and runs the matching combined command that renders all bars and elements in one actionbar update. Command count: the product of all bar `steps` values. Two bars of 20 steps each → 400 commands. For three bars keep steps ≤ 10 (≤ 1 000 commands total) to stay well within Minecraft's function call budget.",
        minecraft = "`executor` is the player selector, typically `\"@a\"`. For each player, Minecraft checks their scores and runs the matching combined command that renders all bars and elements in one actionbar update.",
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(executor = "`executor` is the player selector, typically `\"@a\"`. For each player, Minecraft checks their scores and runs the matching combined command that renders all bars and elements in one actionbar update."),
        returns = "The ordered values produced to generate all display commands for a tick function.",
        example = "pub fn tick() {\nmcfunction! {\n// ... scoreboard math for hp_frame and mp_frame ...\nHudLayout::new(\"my_pack\", 1000)\n.bar(HEALTH, \"hp_frame\", 250)\n.bar(MANA,   \"mp_frame\", 750)\n.broadcast(\"@a\");\n}\n}",
        availability = ["Cargo feature: resourcepack"],
    )]
    ...
    #[sand_macros::api(
        registry = sand_api_contract,
        path = "sand::resourcepack::HudLayout::setup_storage",
        module = "sand::resourcepack",
        kind = "method",
        summary = "Returns a single `data modify storage` command that stores the static element JSON at `{namespace}:hud static`.",
        context = "Returns a single `data modify storage` command that stores the static element JSON at `{namespace}:hud static`. Call this from your load function once to initialize the storage key that [`broadcast_via_storage`] reads every tick. If this layout has no static elements, returns `None`. [`broadcast_via_storage`]: HudLayout::broadcast_via_storage",
        minecraft = "The resourcepack exporter writes version-appropriate assets, bitmap-font providers, and pack metadata for the selected Minecraft profile.",
        use_when = ["Call this from your load function once to initialize the storage key that [`broadcast_via_storage`] reads every tick."],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        returns = "Returns a single `data modify storage` command that stores the static element JSON at `{namespace}:hud static`.",
        example = "pub fn load() {\nmcfunction! {\n// ... other load commands ...\nMY_LAYOUT.setup_storage();\n}\n}",
        availability = ["Cargo feature: resourcepack"],
    )]
    ...
    #[sand_macros::api(
        registry = sand_api_contract,
        path = "sand::resourcepack::HudLayout::broadcast_via_storage",
        module = "sand::resourcepack",
        kind = "method",
        summary = "Storage-based HUD broadcast: O(N₁ + N₂ + … + 1) commands instead of the Cartesian-product O(N₁ × N₂ × …) produced by [`broadcast`].",
        context = "Storage-based HUD broadcast: O(N₁ + N₂ + … + 1) commands instead of the Cartesian-product O(N₁ × N₂ × …) produced by [`broadcast`]. For each bar this emits one `execute if score …` command per frame that writes the frame JSON into `{namespace}:hud barI` in NBT storage. A single `title` command then reads every bar's storage key (and the pre-initialized `static` key for elements) to assemble the final actionbar in one shot. If your layout contains static elements, call [`setup_storage`] from your load function first. Because all bars share the same storage keys, this method must run inside a per-player function so that each player's writes are flushed before the `title` command reads them. The canonical pattern is to call it from an anonymous `run_fn!` block (which Sand wraps in a `execute as @a run function …` call):",
        minecraft = "For each bar this emits one `execute if score …` command per frame that writes the frame JSON into `{namespace}:hud barI` in NBT storage. A single `title` command then reads every bar's storage key (and the pre-initialized `static` key for elements) to assemble the final actionbar in one shot.",
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(executor = "`executor` supplies the executor value used to storage-based HUD broadcast: O(N₁ + N₂ + … + 1) commands instead of the Cartesian-product O(N₁ × N₂ × …) produced by [`broadcast`]."),
        returns = "The ordered values produced to storage-based HUD broadcast: O(N₁ + N₂ + … + 1) commands instead of the Cartesian-product O(N₁ × N₂ × …) produced by [`broadcast`].",
        example = "pub fn tick() {\nmcfunction! {\nexecute as @a run run_fn!({\n// scale / clamp bar objectives …\nMY_LAYOUT.broadcast_via_storage(\"@s\");\n});\n}\n}",
        availability = ["Cargo feature: resourcepack"],
    )]
    ...
    #[sand_macros::api(
        registry = sand_api_contract,
        path = "sand::resourcepack::HudLayout::show_elements",
        module = "sand::resourcepack",
        kind = "method",
        summary = "Generate commands that show only the static elements in this layout (ignoring all bars).",
        context = "Generate commands that show only the static elements in this layout (ignoring all bars). Useful for a load function or any one-shot display of decorative overlays.",
        minecraft = "The resourcepack exporter writes version-appropriate assets, bitmap-font providers, and pack metadata for the selected Minecraft profile.",
        use_when = ["Building HUD bars, HUD elements, textures, or resource-pack output alongside a Sand datapack"],
        avoid_when = ["The project is datapack-only or needs unrelated resource-pack functionality not modeled by Sand"],
        params(target = "`target` provides the entity, block, or command target used to generate commands that show only the static elements in this layout (ignoring all bars)."),
        returns = "The ordered values produced to generate commands that show only the static elements in this layout (ignoring all bars).",
        example = "use sand::prelude::*;\n\nfn demonstrate(hud_layout_value: &sand::resourcepack::HudLayout, target: & str)  {\n    let values = hud_layout_value.show_elements(target);\n}",
        availability = ["Cargo feature: resourcepack"],
    )]
