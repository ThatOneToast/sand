...
fn generated_registrations_build_an_installed_catalog() {
    ...
    assert_eq!(coverage.static_surface_items, 11_010);
    assert_eq!(coverage.pending_item_ceiling, 2_475);
    assert_eq!(coverage.pending_scope_ceiling, 4);
    assert_eq!(coverage.pending_scopes.len(), 4);
