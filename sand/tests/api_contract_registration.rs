...
fn generated_registrations_build_an_installed_catalog() {
    ...
    assert_eq!(coverage.static_surface_items, 10_988);
    assert_eq!(coverage.pending_item_ceiling, 1_656);
    assert_eq!(coverage.pending_scope_ceiling, 3);
    assert_eq!(coverage.pending_scopes.len(), 3);
