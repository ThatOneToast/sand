...
fn generated_registrations_build_an_installed_catalog() {
    ...
    assert_eq!(coverage.static_surface_items, 11_629);
    assert_eq!(coverage.pending_item_ceiling, 6_495);
    assert_eq!(coverage.pending_scope_ceiling, 31);
    assert_eq!(coverage.pending_scopes.len(), 31);
