...
fn generated_registrations_build_an_installed_catalog() {
    ...
    assert_eq!(coverage.static_surface_items, 10_936);
    assert_eq!(coverage.pending_item_ceiling, 559);
    assert_eq!(coverage.pending_scope_ceiling, 2);
    assert_eq!(coverage.pending_scopes.len(), 2);
